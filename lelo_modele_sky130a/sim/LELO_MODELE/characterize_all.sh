#!/bin/bash
#
# Characterization Wrapper - Generate CSV files for all W/L/Corner combinations
...
WIDTHS=(5 10 15 20 25 30 35 40 45 50 80)      # µm - removed 100 (netlist generation fails)
LENGTHS=(0.8 0.9 1.0)                          # µm
CORNERS=(tt ss ff)                             # Process corners - only these have individual Makefile targets

echo "================================================================================="
echo "CHARACTERIZATION CAMPAIGN - All W/L/Corner Combinations"
echo "================================================================================="
...
echo "$(date): Starting characterization campaign" | tee $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "Estimated time: ~${TOTAL} minutes (0.7s per device + corner overhead)" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE

# Run characterization loop
for CORNER in "${CORNERS[@]}"; do
  echo "================================================================================" | tee -a $LOG_FILE
  ...
  for L in "${LENGTHS[@]}"; do
    for W in "${WIDTHS[@]}"; do
      ...
      echo "[${PERCENT}%] W=${W}µm, L=${L}µm, corner=${CORNER}" | tee -a $LOG_FILE
      ...
      if python3 characterize_measured_corner.py $W $L $CORNER >> "$LOG_FILE" 2>&1; then
        ...
        if [ ! -f "$CSV_FILE" ]; then
          # First run - copy header from generated CSV
