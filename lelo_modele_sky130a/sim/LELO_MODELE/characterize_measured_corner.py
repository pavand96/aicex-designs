...
def vsb_to_tb(vsb):
    vb = -vsb  # VB = -VSB (VS=0, VB negative for NMOS body effect)
    if vb >= 0:
        return f"opvb0p{int(round(vb*10)):01d}"
    else:
        return f"opvbm0p{int(round(-vb*10)):01d}"
...
work_dir = os.path.expanduser("~/pro/aicex/ip/lelo_modele_sky130a/work")
...
# Regenerate SPICE netlist from schematic (use -B to force Make rebuild)
xsch_result = subprocess.run(['make', 'xsch', 'CELL=LELO_MODELE'],
...
if xsch_result.returncode != 0:
    ...
    exit(1)
print(f"✓ Netlist regenerated via make xsch CELL=LELO_MODELE")

# ============================================================================
# PHASE 1: Run nested DC sweep
# ============================================================================
print(f"\n[PHASE 1/3] Running nested DC sweep (VGS, VDS = 0.01V steps, VSB={VSB:.1f}V)...")
t0 = time.time()

OUTPUT_DIR = f'output_{TB_NAME}'

# Remove output dir to force fresh simulation
if os.path.exists(OUTPUT_DIR):
    shutil.rmtree(OUTPUT_DIR)

# Use corner parameter and specific TB in make command
sim_result = subprocess.run(['make', CORNER_MAKE, f'TB={TB_NAME}'], capture_output=True, timeout=600)
if sim_result.returncode != 0:
    print(f"❌ DC Simulation failed (corner={CORNER} → make {CORNER_MAKE} TB={TB_NAME})")
    print(sim_result.stderr.decode() if sim_result.stderr else "")
    exit(1)

# Find raw file
find_result = subprocess.run(['find', OUTPUT_DIR, '-name', '*.raw', '-printf', '%T@ %p\n'],
...
idx_vs = var_names['v(vs)']
idx_id = var_names['i(vds)']   # Current through VDS source = drain current

# Body effect: VTH shifts with VSB
# VTH(VSB) = VTH0 + gamma*(sqrt(2*PHI_F + VSB) - sqrt(2*PHI_F))
# sky130 NMOS approx: gamma~0.4, PHI_F~0.4V
GAMMA  = 0.4
PHI_F  = 0.4
VTH_EFF = VTH + GAMMA * (np.sqrt(2*PHI_F + VSB) - np.sqrt(2*PHI_F))
print(f"✓ Effective VTH at VSB={VSB:.2f}V: {VTH_EFF:.3f}V (VTH0={VTH:.3f}V)")

dc_grid = {}
dc_points = []

for pt in raw_data:
    vs  = pt[idx_vs]              # source absolute voltage (should be ~0)
    vgs = pt[idx_vg] - vs         # gate-to-source
    vds = pt[idx_vd] - vs         # drain-to-source
    ids = pt[idx_id]
    id_val = abs(ids)
    vov = vgs - VTH_EFF
...
# Filter to saturation (NMOS: VGS > VTH_EFF, VDS >= VOV)
ac_points = []
...
# e.g. characterize_2d_tt_vsb0p0.csv, characterize_2d_tt_vsb0p3.csv ...
